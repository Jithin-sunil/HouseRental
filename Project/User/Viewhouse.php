...
if(isset($_GET['Bid']))
{
	$insQry="insert into tbl_booking (house_id,user_id,booking_date)values('".$_GET['hid']."','".$_SESSION['uid']."',curdate())";
	if($Con->query($insQry))
	{
		?>
        <script>
...
<form id="form1" name="form1" method="post" action="">
  ...
  <table width="200" border="1" id="result">
    <tr>
     ...
     <?php
	$i=0;
	$selQry="select * from tbl_house h inner join tbl_place p on h.place_id = p.place_id inner join tbl_district d on p.district_id=d.district_id inner join tbl_housetype ht on h.housetype_id = ht.housetype_id inner join tbl_type t on h.type_id = t.type_id where h.type_id=1 ";
	$row=$Con->query($selQry);
	while($data=$row->fetch_assoc())
	{
		$i++;
...
<script>
    function gethouse(did) 
    {
        ...
        $.ajax({
        url:"../Assets/AjaxPages/AjaxHouse.php?did="+did,
        success: function(html){
            $("#result").html(html);
        }
        });
